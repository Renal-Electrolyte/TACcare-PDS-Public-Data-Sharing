...
easypackages::libraries("here", "tidyverse", "flexdashboard", 
                        ...
                        "boot", "ggpubr")
df_bl_comp <- read_excel(here::here("ema/derived_data/baseline_composite_scores.xlsx"))

source(here("Primary paper","src","fcn_output.R"))
sdi <- readxl::read_excel(here("Baseline_Cytokine_vs_Symptoms/derived_data/demo_sub_com_sdi.xlsx"))

# import primary outcomes (Symptoms)
primary_deri <- here("Primary paper/rdata")
symptoms <- readRDS(here(primary_deri,"symptoms_imp.Rds"))

# import demographic data 
primary_deri <- here("Primary paper/rdata")
demog <- readRDS(here(primary_deri,"demog.RDS"))

## Prepare data for analysis
symptoms_bl <- symptoms |> 
  select(id:randgrp_f, date, time,
         dep_imp, bpi_imp, fs_imp,
         bl_symptom, psqi_imp, gad_imp,
         ssTotal_imp, paTotal_imp, mcs, pcs) |>
  mutate(Timepoint = case_when(
    time == 1 ~ as.character("T-0"),
    time == 2 ~ as.character("T-1"),
    time == 3 ~ as.character("T-2"),
    time == 4 ~ as.character("T-3")
  )) |>
  filter(time == 1)

meds <- readRDS(here("Primary paper/rdata/meds_antiDep_opioid_pt.RDS"))

source(here("ema","src","meds.R"))

ktv <- eligible_patients %>% 
  select(id, screen_id, site) %>%
  left_join(info) %>%
  filter(redcap_event_name == "Baseline (Arm 1: Pre-randomization)" & 
           redcap_repeat_instrument == "Labs") %>%
  group_by(id) %>%
  summarise(ktv = mean(ktv, na.rm = TRUE)) %>%
  ungroup() 

cardio_diab <- demog |>
  select(id, cva_yn, cpd_yn, diabetes_wo_damage) |>
  distinct(id, .keep_all = TRUE) |>
  mutate(id = as.numeric(id)) |>
  mutate(across(cva_yn:diabetes_wo_damage, as.factor))
```
...
df_bl_comp <- df_bl_comp |> 
  mutate(dialysis_start_time = hms::as_hms(dialysis_start_time),
    `Dialysis Shift` = case_when(
    dialysis_start_time < hms::as_hms("8:00:00") ~ "Morning",
    dialysis_start_time >= hms::as_hms("8:00:00") & 
      dialysis_start_time < hms::as_hms("13:00:00") ~ "Midday",
    dialysis_start_time >= hms::as_hms("13:00:00") ~ "Evening",
    .default = "NA"
  ),
  dialysis_order = case_when(
    dialysis_sched == "M/W/F" & call_day == "Friday" |
      dialysis_sched == "T/Th/S" & call_day == "Saturday" ~ "Day 5 (HD)",
    dialysis_sched == "M/W/F" & call_day %in%  c("Saturday") |
      dialysis_sched == "T/Th/S" & call_day %in% c("Sunday") ~ "Day 6 (Non-HD)",
    dialysis_sched == "M/W/F" & call_day %in%  c("Sunday") |
      dialysis_sched == "T/Th/S" & call_day %in% c("Monday") ~ "Day 7 (Non-HD)",
    dialysis_sched == "M/W/F" & call_day %in%  c("Monday") |
      dialysis_sched == "T/Th/S" & call_day %in% c("Tuesday") ~ "Day 1 (HD)",
    dialysis_sched == "M/W/F" & call_day %in%  c("Tuesday") |
      dialysis_sched == "T/Th/S" & call_day %in% c("Wednesday") ~ "Day 2 (Non-HD)",
    dialysis_sched == "M/W/F" & call_day %in%  c("Wednesday") |
      dialysis_sched == "T/Th/S" & call_day %in% c("Thursday") ~ "Day 3 (HD)",
    dialysis_sched == "M/W/F" & call_day %in%  c("Thursday") |
      dialysis_sched == "T/Th/S" & call_day %in% c("Friday") ~ "Day 4 (Non-HD)",
    .default = "NA"
  ),
  sequential_time_order = case_when(
    dialysis_order == "Day 1 (HD)" & timeofday == 1 ~ 1,
    dialysis_order == "Day 1 (HD)" & timeofday == 2 ~ 2,
    dialysis_order == "Day 1 (HD)" & timeofday == 3 ~ 3,
    dialysis_order == "Day 1 (HD)" & timeofday == 4 ~ 4,
    dialysis_order == "Day 2 (Non-HD)" & timeofday == 1 ~ 5,
    dialysis_order == "Day 2 (Non-HD)" & timeofday == 2 ~ 6,
    dialysis_order == "Day 2 (Non-HD)" & timeofday == 3 ~ 7,
    dialysis_order == "Day 2 (Non-HD)" & timeofday == 4 ~ 8,
    dialysis_order == "Day 3 (HD)" & timeofday == 1 ~ 9,
    dialysis_order == "Day 3 (HD)" & timeofday == 2 ~ 10,
    dialysis_order == "Day 3 (HD)" & timeofday == 3 ~ 11,
    dialysis_order == "Day 3 (HD)" & timeofday == 4 ~ 12,
    dialysis_order == "Day 4 (Non-HD)" & timeofday == 1 ~ 13,
    dialysis_order == "Day 4 (Non-HD)" & timeofday == 2 ~ 14,
    dialysis_order == "Day 4 (Non-HD)" & timeofday == 3 ~ 15,
    dialysis_order == "Day 4 (Non-HD)" & timeofday == 4 ~ 16,
    dialysis_order == "Day 5 (HD)" & timeofday == 1 ~ 17,
    dialysis_order == "Day 5 (HD)" & timeofday == 2 ~ 18,
    dialysis_order == "Day 5 (HD)" & timeofday == 3 ~ 19,
    dialysis_order == "Day 5 (HD)" & timeofday == 4 ~ 20,
    dialysis_order == "Day 6 (Non-HD)" & timeofday == 1 ~ 21,
    dialysis_order == "Day 6 (Non-HD)" & timeofday == 2 ~ 22,
    dialysis_order == "Day 6 (Non-HD)" & timeofday == 3 ~ 23,
    dialysis_order == "Day 6 (Non-HD)" & timeofday == 4 ~ 24,
    dialysis_order == "Day 7 (Non-HD)" & timeofday == 1 ~ 25,
    dialysis_order == "Day 7 (Non-HD)" & timeofday == 2 ~ 26,
    dialysis_order == "Day 7 (Non-HD)" & timeofday == 3 ~ 27,
    dialysis_order == "Day 7 (Non-HD)" & timeofday == 4 ~ 28,
    .default = NA),
  dialysis_order_cont = case_when(dialysis_order == "Day 5 (HD)" ~ 5,
                                  dialysis_order == "Day 6 (Non-HD)" ~ 6,
                                  dialysis_order == "Day 7 (Non-HD)" ~ 7,
                                  dialysis_order == "Day 1 (HD)" ~ 1,
                                  dialysis_order == "Day 2 (Non-HD)" ~ 2,
                                  dialysis_order == "Day 3 (HD)" ~ 3,
                                  dialysis_order == "Day 4 (Non-HD)" ~ 4,
  ),
  dialysis_non_first_second = case_when(dialysis_order == "Day 5 (HD)" |
                                        dialysis_order == "Day 1 (HD)" |
                                        dialysis_order == "Day 3 (HD)" ~ "Dialysis days",
                                        dialysis_order == "Day 2 (Non-HD)" |
                                        dialysis_order == "Day 4 (Non-HD)" |
                                        dialysis_order == "Day 6 (Non-HD)" |
                                          dialysis_order == "Day 7 (Non-HD)" ~ "Non-dialysis days"))

df_bl_comp <- df_bl_comp |> 
  mutate(dialysis_start_time = hms::as_hms(dialysis_start_time),
         dialysis_stop_time = hms::as_hms(dialysis_stop_time),
         call_start_time = hms::as_hms(call_start_time),
         call_end_time = hms::as_hms(call_end_time),
         eval_time_flag = case_when(
           call_end_time < dialysis_start_time ~ "before_dialysis" ,
           call_start_time > dialysis_stop_time ~ "after_dialysis" ,
           call_start_time >= (dialysis_start_time - hms::hms(0, 6, 0)) &
             call_end_time <= (dialysis_stop_time + hms::hms(0, 6, 0)) ~ "during_dialysis",
           .default = NA
         ))

# Used for dialysis shift visualizations
dialysis_order_df <- df_bl_comp |>
    filter(dialysis_order != "NA") |>
  group_by(timeofday, dialysis_order, `Dialysis Shift`) |> 
  summarise(across(`Negative Mood` : `Alert/Cognitive`, mean, na.rm = T)) |>
  pivot_longer(cols = `Negative Mood` : `Alert/Cognitive`, 
               names_to = "Composite score", 
               values_to = "Mean") |>
  ungroup()

# Used for daily/diurnal dialysis visualizations
diurnal_daily_dialysis <- df_bl_comp |>
    filter(dialysis_order != "NA") |>
...
dialysis_order_df_by_day_mean <- df_bl_comp |>
  group_by(dialysis_order_cont, dialysis_day_flag) |> 
  summarise(across(`Negative Mood` : `Alert/Cognitive`, mean, na.rm = T)) |>
  pivot_longer(cols = `Negative Mood` : `Alert/Cognitive`, 
               names_to = "Composite score", 
               values_to = "Mean") |>
  ungroup() |>
  group_by(`Composite score`) |>
  arrange(dialysis_order_cont) |>
  mutate(diff = Mean - lag(Mean)) |>
  ungroup()

dialysis_order_df_by_day_min <- df_bl_comp |>
  group_by(dialysis_order_cont, dialysis_day_flag) |> 
  summarise(across(`Negative Mood` : `Alert/Cognitive`, min, na.rm = T)) |>
  pivot_longer(cols = `Negative Mood` : `Alert/Cognitive`, 
               names_to = "Composite score", 
               values_to = "Min") |>
  ungroup() 

dialysis_order_df_by_day_max <- df_bl_comp |>
  group_by(dialysis_order_cont, dialysis_day_flag) |> 
  summarise(across(`Negative Mood` : `Alert/Cognitive`, max, na.rm = T)) |>
  pivot_longer(cols = `Negative Mood` : `Alert/Cognitive`, 
               names_to = "Composite score", 
               values_to = "Max") |>
  ungroup() 

daily_dialysis_changes <- dialysis_order_df_by_day_mean |> 
  inner_join(dialysis_order_df_by_day_min, 
             by = c("dialysis_order_cont", "dialysis_day_flag", "Composite score")) |>
  inner_join(dialysis_order_df_by_day_max, 
             by = c("dialysis_order_cont", "dialysis_day_flag", "Composite score"))

daily_dialysis_changes_by_shift <- df_bl_comp |>
  group_by(dialysis_order_cont, dialysis_day_flag, `Dialysis Shift`) |> 
  summarise(across(`Negative Mood` : `Alert/Cognitive`, mean, na.rm = T)) |>
  pivot_longer(cols = `Negative Mood` : `Alert/Cognitive`, 
               names_to = "Composite score", 
               values_to = "Mean") |>
...
#### Comparing dialysis, non-dialysis, with first and second days of non dialysis weekends
dial_non_weekends <- df_bl_comp |>
  group_by(timeofday, dialysis_non_first_second) |> 
  summarise(across(`Negative Mood` : `Alert/Cognitive`, mean, na.rm = T)) |>
  pivot_longer(cols = `Negative Mood` : `Alert/Cognitive`, 
               names_to = "Composite score", 
               values_to = "Mean") |>
  ungroup() |>
  mutate(timeofday = as.numeric(timeofday))

```

```{r}
post_dial <- df_bl_comp |>
  filter(eval_time_flag == "after_dialysis") |>
  na.omit() |>
...
#### Average composite score by day
...
daily_dialysis_changes |>
  ...
  filter(`Composite score` == "Alert/Cognitive") |>
  ggplot(aes(x = dialysis_order_cont, y = Mean)) +
  geom_point(size = 2) +
  geom_path() +
  labs(x = "Day", y = "Mean (Alert/Cognitive)") +
  theme_pubr() +
  ...
  scale_x_continuous(breaks = c(1, 2, 3, 4, 5, 6, 7), labels = c("Day 1 (HD)",
                                                                 ...
                                                                 "Day 6 (Non-HD)",
                                                                 "Day 7 (Non-HD)")) +
  scale_y_continuous(limits = c(4.7, 5.1)) +
  geom_vline(xintercept = c(1, 2, 3, 4, 5, 6, 7), linetype = "dashed", color = "blue")
```

#### Average composite score by day and dialysis shift
```{r, fig.align='center', fig.width=12, fig.height=4}
daily_dialysis_changes_by_shift |>
  rename(`Dialysis day flag` = dialysis_day_flag) |>
  mutate(`Dialysis day flag` = case_when(`Dialysis day flag` == "dialysis_day" ~ "Dialysis day",
                                         `Dialysis day flag` == "not_dialysis_day" ~ "Non-dialysis day")) %>%
  filter(`Composite score` == "Alert/Cognitive",
         `Dialysis Shift` != 'Evening') |>
  ggplot(aes(x = dialysis_order_cont, y = Mean)) +
  geom_point(size = 2) +
  geom_path(aes(group = `Dialysis Shift`, color = `Dialysis Shift`)) +
  labs(x = "Day", y = "Mean (Alert/Cognitive)") +
  theme_pubr() +
  theme(legend.position = "top", 
        legend.text = element_text(size = 11),
  ...
  scale_x_continuous(breaks = c(1, 2, 3, 4, 5, 6, 7), labels = c("Day 1 (HD)",
                                                                 ...
                                                                 "Day 6 (Non-HD)",
                                                                 "Day 7 (Non-HD)")) +
  scale_y_continuous(limits = c(4.7, 5.2)) +
  geom_vline(xintercept = c(1, 2, 3, 4, 5, 6, 7), linetype = "dashed", color = "blue")
```

#### Average composite score by time of day
```{r, fig.align='center', fig.width=12, fig.height=4}
...
labels_df <- data.frame(
  ...
  label = c("Day 1 (HD)","Day 2 (Non-HD)","Day 3 (HD)","Day 4 (Non-HD)","Day 5 (HD)", 
                                                                 "Day 6 (Non-HD)",
                                                                 "Day 7 (Non-HD)")
)

diurnal_daily_dialysis |>
  filter(`Composite score` == "Alert/Cognitive") |>
  ggplot(aes(x = sequential_time_order, y = Mean)) +
  geom_point(size = 2) +
  geom_path() +
  labs(x = "Time of day", y = "Mean (Alert/Cognitive)") +
  theme_pubr() +
...
#### Compare dialysis, non-dialysis and weekends
```{r, fig.align='center', fig.width=12, fig.height=4}
dial_non_weekends |>
  mutate(dialysis_non_first_second = fct_relevel(dialysis_non_first_second,
                                                c("dialysis_days", 
                                                  "non_dialysis_week_days", 
                                                  "first_non_dialysis_weekend_day", 
                                                  "second_non_dialysis_weekend_day"))) |>
  filter(`Composite score` == "Alert/Cognitive") |>
  ggplot(aes(x = timeofday, y = Mean)) +
  geom_point(size = 2) +
  geom_path() +
  facet_wrap(~ dialysis_non_first_second, nrow = 1) +
  labs(x = "Time of day", y = "Mean (Alert/Cognitive)") +
  theme_pubr() +
  theme(legend.position = "top", 
        legend.text = element_text(size = 11),
        axis.text.x = element_text(hjust = 1, size = 11)) +
  geom_text(aes(label = round(Mean, 2)), vjust = - 1, hjust = 0.3, size = 4) +
  scale_y_continuous(limits = c(4.7, 5.2))
``` 
...
dialysis_order_df |>
  ...
  geom_point(size = 2) +
  geom_path(aes(group = `Dialysis Shift`, color = `Dialysis Shift`)) +
  facet_grid( . ~ dialysis_order ) +
  labs(x = "Time of day", y = "Mean composite score (Alert/Cognitive)") +
  theme_pubr() +
  theme(legend.position = "bottom", axis.text.x = element_text( hjust = 1, size = 9)) +
  geom_text(aes(label = round(Mean, 2)), vjust = - 1, hjust = 0.3, size = 4)
```
...
daily_dialysis_changes |>
  ...
  mutate(`Dialysis day flag` = case_when(`Dialysis day flag` == "dialysis_day" ~ "Dialysis day",
                                         `Dialysis day flag` == "not_dialysis_day" ~ "Non-dialysis day")) %>%
  filter(`Composite score` == "Negative Mood") |>
  ggplot(aes(x = dialysis_order_cont, y = Mean)) +
  geom_point(size = 2) +
  geom_path() +
  labs(x = "Day", y = "Mean (Negative Mood)") +
  theme_pubr() +
  theme(legend.position = "top", 
        legend.text = element_text(size = 11),
        axis.text.x = element_text(hjust = 1, size = 11, angle = 25)) +
  geom_text(aes(label = round(Mean, 2)), vjust = - 0.8, hjust = -0.2, size = 4) +
  scale_x_continuous(breaks = c(1, 2, 3, 4, 5, 6, 7), labels = c("Day 1 (HD)",
                                                                 "Day 2 (Non-HD)",
                                                                 "Day 3 (HD)",
                                                                 "Day 4 (Non-HD)",
...
labels_df <- data.frame(
  x = c(2, 6, 10, 14, 18, 22, 26),  # X-coordinates for each day
  y = rep(3.3, 7),                    # Y-coordinate for all labels
  label = c("Day 1 (HD)",
                                                                 "Day 2 (Non-HD)",
                                                                 "Day 3 (HD)",
                                                                 "Day 4 (Non-HD)",
...
diurnal_daily_dialysis |>
  ...
  geom_point(size = 2) +
  geom_path() +
  labs(x = "Time of day", y = "Mean (Negative Mood)") +
  theme_pubr() +
  theme(legend.position = "bottom", 
        legend.text = element_text(size = 11),
        axis.text.x = element_text(hjust = 1, size = 11)) +
...
dial_non_weekends |>
  ...
  theme(legend.position = "top", 
        legend.text = element_text(size = 11),
        axis.text.x = element_text(hjust = 1, size = 11)) +
  geom_text(aes(label = round(Mean, 2)), vjust = - 1, hjust = 1, size = 4) +
  scale_y_continuous(limits = c(2, 3))
``` 
...
dialysis_order_df |>
  ...
  geom_path(aes(group = `Dialysis Shift`, color = `Dialysis Shift`)) +
  facet_grid( . ~ dialysis_order ) +
  labs(x = "Time of day", y = "Mean composite score (Negative Mood)") +
  theme_pubr() +
  theme(legend.position = "bottom", axis.text.x = element_text( hjust = 1, size = 9)) +
  geom_text(aes(label = round(Mean, 2)), vjust = - 1, hjust = 0.3, size = 4)
```
...
daily_dialysis_changes |>
  ...
  mutate(`Dialysis day flag` = case_when(`Dialysis day flag` == "dialysis_day" ~ "Dialysis day",
                                         `Dialysis day flag` == "not_dialysis_day" ~ "Non-dialysis day")) %>%
  filter(`Composite score` == "Positive Mood") |>
  ggplot(aes(x = dialysis_order_cont, y = Mean)) +
  geom_point(size = 2) +
  geom_path() +
  labs(x = "Day", y = "Mean (Positive Mood)") +
  theme_pubr() +
  theme(legend.position = "top", 
        legend.text = element_text(size = 11),
        axis.text.x = element_text(hjust = 1, size = 11, angle = 25)) +
...
daily_dialysis_changes_by_shift |>
  ...
  scale_x_continuous(breaks = c(1, 2, 3, 4, 5, 6, 7), labels = c("Day 1 (HD)",
                                                                 ...
                                                                 "Day 6 (Non-HD)",
                                                                 "Day 7 (Non-HD)")) +
  scale_y_continuous(limits = c(3.7, 4.9)) +
  geom_vline(xintercept = c(1, 2, 3, 4, 5, 6, 7), linetype = "dashed", color = "blue")
```
...
diurnal_daily_dialysis |>
  ...
  geom_point(size = 2) +
  geom_path() +
  labs(x = "Time of day", y = "Mean (Positive Mood)") +
  theme_pubr() +
  theme(legend.position = "bottom", 
        legend.text = element_text(size = 11),
        axis.text.x = element_text(hjust = 1, size = 11)) +
...
#### By dialysis shift-post dialysis
```{r, fig.align='center', fig.width=14, fig.height=6}
dialysis_order_df |>
  filter(`Composite score` == "Positive Mood",
         `Dialysis Shift` != "Evening") |>
  ...
  geom_path(aes(group = `Dialysis Shift`, color = `Dialysis Shift`)) +
  facet_grid( . ~ dialysis_order ) +
  labs(x = "Time of day", y = "Mean composite score (Positive Mood)") +
  theme_pubr() +
  theme(legend.position = "bottom", axis.text.x = element_text( hjust = 1, size = 9)) +
  geom_text(aes(label = round(Mean, 2)), vjust = - 1, hjust = 0.3, size = 4)
```
...
daily_dialysis_changes |>
  ...
  mutate(`Dialysis day flag` = case_when(`Dialysis day flag` == "dialysis_day" ~ "Dialysis day",
                                         `Dialysis day flag` == "not_dialysis_day" ~ "Non-dialysis day")) %>%
  filter(`Composite score` == "Sleepiness/Fatigue") |>
  ggplot(aes(x = dialysis_order_cont, y = Mean)) +
  geom_point(size = 2) +
  geom_path() +
  labs(x = "Day", y = "Mean (Sleepiness/Fatigue)") +
  theme_pubr() +
  theme(legend.position = "top", 
        legend.text = element_text(size = 11),
        axis.text.x = element_text(hjust = 1, size = 11, angle = 25)) +
...
labels_df <- data.frame(
  x = c(2, 6, 10, 14, 18, 22, 26),  # X-coordinates for each day
  y = rep(4.7, 7),                    # Y-coordinate for all labels
  label = c("Day 1 (HD)",
                                                                 "Day 2 (Non-HD)",
                                                                 "Day 3 (HD)",
                                                                 "Day 4 (Non-HD)",
...
diurnal_daily_dialysis |>
  ...
  geom_point(size = 2) +
  geom_path() +
  labs(x = "Time of day", y = "Mean (Sleepiness/Fatigue)") +
  theme_pubr() +
  theme(legend.position = "bottom", 
        legend.text = element_text(size = 11),
        axis.text.x = element_text(hjust = 1, size = 11)) +
  geom_vline(xintercept = c(4, 8, 12, 16, 20, 24, 28), linetype = "dashed", color = "blue") +
  scale_x_continuous(breaks = seq(1:28)) +
  scale_y_continuous(limits = c(3.2, 4.7)) +
  geom_text(aes(label = round(Mean, 2)), vjust = - 1, hjust = -0.1, size = 4) +
  geom_label(data = labels_df, aes(x = x, y = y, label = label), size = 4, 
             fill = "white", color = "black", label.padding = unit(0.25, "lines"),
             label.r = unit(0.2, "lines"), label.size = 0.25)
...
dial_non_weekends |>
  ...
  theme(legend.position = "top", 
        ...
        axis.text.x = element_text(hjust = 1, size = 11)) +
  geom_text(aes(label = round(Mean, 2)), vjust = - 1, hjust = 0.3, size = 4) +
  scale_y_continuous(limits = c(3.2, 4.5))
``` 
...
dialysis_order_df |>
  ...
  geom_path(aes(group = `Dialysis Shift`, color = `Dialysis Shift`)) +
  facet_grid( . ~ dialysis_order ) +
  labs(x = "Time of day", y = "Mean composite score (Sleepiness/Fatigue)") +
  theme_pubr() +
  theme(legend.position = "bottom", axis.text.x = element_text( hjust = 1, size = 9)) +
  geom_text(aes(label = round(Mean, 2)), vjust = - 1, hjust = 0.3, size = 4)
```
...
### **dialysis vs. non-dialysis controling for cvd, copd, and diabetes**
...
model_formula <- as.formula(glue::glue("{y} ~ 1 + dialysis_non_first_second + gender + race + 
                                       cva_yn + cpd_yn + diabetes_wo_damage + screen_age + (1 |id)"))
...
### **Morning vs. Midday/Evening**


#### Pairwise comparisons

```{r, Mixed-effect models by dialysis shift}
library(lme4)
library(broom)
library(broom.mixed)
library(marginaleffects)
...
fits <- ys |>
  map(~ fitMod(post_dial %>% mutate(`Dialysis Shift` = ifelse(`Dialysis Shift` == "Evening" | 
                                     `Dialysis Shift` == "Midday", 
                                   "Midday or Evening", 
                                   `Dialysis Shift`)), .))
...
#### Overall model p-values
```{r}
overall_p <- map_dfr(fits, ~ get_overall_p(.),.id="y") |>
  rename(CompositeScore = y) |>
  mutate(CompositeScore = case_when(
    CompositeScore == "`Negative Mood`" ~ "Negative Mood",
    CompositeScore == "`Sleepiness/Fatigue`" ~ "Sleepiness/Fatigue",
    CompositeScore == "`Positive Mood`" ~ "Positive Mood",
    CompositeScore == "`Alert/Cognitive`" ~ "Alert/Cognitive"
  ), 
...
post_dial |> 
  mutate(`Dialysis Shift` = ifelse(`Dialysis Shift` == "Evening" | 
                                   ...
                                   `Dialysis Shift`)) |>
  pivot_longer(cols = `Negative Mood`:`Alert/Cognitive`, names_to = "CompositeScore") |>
  group_by(CompositeScore, `Dialysis Shift`) |>
  summarise(mean = mean(value, na.rm = TRUE), sd = sd(value, na.rm = TRUE)) |>
  mutate(mean_sd = glue::glue("{round(mean, 1)} ({round(sd, 1)})")) |>
  select(CompositeScore, `Dialysis Shift`, mean_sd) |>
  pivot_wider(names_from = 'Dialysis Shift', values_from = mean_sd) |>
  left_join(overall_p, by = "CompositeScore") |>
  select(CompositeScore, Morning, `Midday or Evening`, p.value) |>
  mutate(CompositeScore = as.factor(CompositeScore)) |>
  mykable(n.left = 2)
```

#### Comparing Morning vs. Midday/Evening by Dialysis
...
fits_dialysis <- ys |>
  map(~ fitMod(post_dial %>% mutate(`Dialysis Shift` = ifelse(`Dialysis Shift` == "Evening" | 
                                   ...
                                   "Midday or Evening", 
                                   `Dialysis Shift`)) %>%
                 filter(dialysis_non_first_second == "Dialysis days"), .))

names(fits_dialysis) <- ys

fits_non_dialysis <- ys |>
  map(~ fitMod(post_dial %>% mutate(`Dialysis Shift` = ifelse(`Dialysis Shift` == "Evening" | 
                                     `Dialysis Shift` == "Midday", 
                                   "Midday or Evening", 
                                   `Dialysis Shift`)) %>%
                 filter(dialysis_non_first_second == "Non-dialysis days"), .))

names(fits_non_dialysis) <- ys

# Get the differences in means
mean_diff_dialysis <- fits_dialysis %>% 
  map_dfr(~adjMod(.x)
          ,.id="y"
          ) |>
  mutate(p.value = round(p.value, 3))

mean_diff_non_dialysis <- fits_non_dialysis %>% 
  map_dfr(~adjMod(.x)
          ,.id="y"
          ) |>
  mutate(p.value = round(p.value, 3))
...
mean_diff_dialysis |>
  select(y, contrast, estimate, p.value, conf.low : conf.high) |>
  mutate(dialysis = "dialysis") |> 
  rbind(mean_diff_non_dialysis |>
          select(y, contrast, estimate, p.value, conf.low : conf.high) |>
          mutate(dialysis = "non-dialysis")) |>
  mykable(n.left = 2) 
```

```{r}
overall_p_dialysis <- map_dfr(fits_dialysis, ~ get_overall_p(.),.id="y") |>
  rename(CompositeScore = y) |>
  mutate(CompositeScore = case_when(
    CompositeScore == "`Negative Mood`" ~ "Negative Mood",
    CompositeScore == "`Sleepiness/Fatigue`" ~ "Sleepiness/Fatigue",
  ...
  p.value = round(p.value, 5)) |>
  mutate(dialysis_non_first_second = "Dialysis days")

overall_p_non_dialysis <- map_dfr(fits_non_dialysis, ~ get_overall_p(.),.id="y") |>
  rename(CompositeScore = y) |>
  mutate(CompositeScore = case_when(
    CompositeScore == "`Negative Mood`" ~ "Negative Mood",
    CompositeScore == "`Sleepiness/Fatigue`" ~ "Sleepiness/Fatigue",
  ...
  p.value = round(p.value, 5)) |>
  mutate(dialysis_non_first_second = "Non-dialysis days")

overall_p <- overall_p_dialysis %>% rbind(overall_p_non_dialysis)
...
post_dial |> 
  mutate(`Dialysis Shift` = ifelse(`Dialysis Shift` == "Evening" | 
                                   ...
                                   `Dialysis Shift`)) |>
  pivot_longer(cols = `Negative Mood`:`Alert/Cognitive`, names_to = "CompositeScore") |>
  group_by(dialysis_non_first_second, CompositeScore, `Dialysis Shift`) |>
  summarise(mean = mean(value, na.rm = TRUE), sd = sd(value, na.rm = TRUE)) |>
  mutate(mean_sd = glue::glue("{round(mean, 1)} ({round(sd, 1)})")) |>
  select(dialysis_non_first_second, CompositeScore, `Dialysis Shift`, mean_sd) |>
  pivot_wider(names_from = 'Dialysis Shift', values_from = mean_sd) |>
  left_join(overall_p, by = c("CompositeScore", "dialysis_non_first_second")) |>
  select(CompositeScore, Morning, `Midday or Evening`, p.value) |>
  mutate(CompositeScore = as.factor(CompositeScore)) |>
  mykable(n.left = 2)
```
...
### **Morning vs. Midday shifts**


#### Pairwise comparisons

```{r, Mixed-effect models by dialysis shift excluding evening time}
library(lme4)
library(broom)
library(broom.mixed)
library(marginaleffects)
...
fits <- ys |>
  map(~ fitMod(post_dial |> filter(`Dialysis Shift` != "Evening"), .))
...
post_dial |> 
  ...
  pivot_wider(names_from = 'Dialysis Shift', values_from = mean_sd) |>
  left_join(overall_p, by = "CompositeScore") |>
  select(CompositeScore, Morning, Midday, p.value) |>
  mutate(CompositeScore = as.factor(CompositeScore)) |>
  mykable(n.left = 2)
```
